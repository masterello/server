...
public class EmailVerifiedChangedEventListener extends MasterelloEventListener<EmailVerifiedChangedEvent> {


    private final WorkerInfoRepository workerInfoRepository;

    @Override
    protected void processEvent(EmailVerifiedChangedEvent event) {
        ...
        if(isWorker(user)) {
            ...
            if(verified != workerInfo.isVerified()) {
                workerInfo.setVerified(verified);
